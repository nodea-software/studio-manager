...
router.get('/', function(req, res) {
    ...
    models.E_environment.findAndCountAll(query).then(function(e_environments) {
        answer["e_environments".substring(2)] = e_environments.rows || [];
        answer.totalCount = e_environments.count;
        answer.rowsCount = answer["e_environments".substring(2)].length;
...
router.get('/:id', function(req, res) {
    ...
    var id_e_environment = parseInt(req.params.id);
    ...
    var where = {id: id_e_environment};
    for (var field in req.query)
        if ((field.indexOf('f_') == 0 && attributes[field]) || field.indexOf('fk_id_') == 0)
            where[field] = req.query[field];
    query.where = where;

    models.E_environment.findOne(query).then(function(e_environment) {
        if (!e_environment) {
            answer.error = "No e_environment with ID "+id_e_environment;
            return res.status(404).json(answer);
        }
        answer["e_environment".substring(2)] = e_environment;
...
router.get('/:id/:association', function(req, res) {
    ...
    var id_e_environment = req.params.id;
    ...
    models.E_environment.findOne({
        ...
        answer[association] = e_environment[include.as];
    ...
    models.E_environment.create(createObject).then(function(e_environment) {
        ...
        for (var prop in req.body)
            if (prop.indexOf('r_') == 0) {
                if (e_environment['set'+entity_helper.capitalizeFirstLetter(prop)] !== 'undefined')
                    associationPromises.push(e_environment['set'+entity_helper.capitalizeFirstLetter(prop)](req.body[prop]));
                else
                    console.error("API: Couldn't set association.\nAPI: e_environment.set"+entity_helper.capitalizeFirstLetter(prop)+"() is undefined.");
            }
...
router.put('/:id', function(req, res) {
    ...
    // Fetch e_environment to update
    models.E_environment.findOne({where: {id: id_e_environment}}).then(function(e_environment) {
        if (!e_environment) {
            answer.error = "No e_environment with ID "+id_e_environment;
            return res.status(404).json(answer);
        }

        // Update e_environment
        e_environment.update(updateObject, {where: {id: id_e_environment}}).then(function() {
            answer["e_environment".substring(2)] = e_environment;
...
router.delete('/:id', function(req, res) {
    ...
    models.E_environment.destroy({where: {id: id_e_environment}}).then(function() {
        res.status(200).end();
    }).catch(function(err){
        answer.error = err;
        res.status(500).json(answer);
