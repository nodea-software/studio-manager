...
router.get('/list', block_access.actionAccessMiddleware("synchro_credentials", "read"), function(req, res) {
    res.render('e_synchro_credentials/list');
});

router.post('/datalist', block_access.actionAccessMiddleware("synchro_credentials", "read"), function(req, res) {
    filterDataTable("E_synchro_credentials", req.body).then(function(rawData) {
        entity_helper.prepareDatalistResult('e_synchro_credentials', rawData, req.session.lang_user).then(function(preparedData) {
...
router.post('/subdatalist', block_access.actionAccessMiddleware("synchro_credentials", "read"), function(req, res) {
    ...
    models.E_synchro_credentials.findOne({
        ...
        e_synchro_credentials['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]().then(function(count) {
            var rawData = {
                recordsTotal: count,
                recordsFiltered: count,
                data: []
            };
            for (var i = 0; i < e_synchro_credentials[subentityAlias].length; i++)
...
router.get('/show', block_access.actionAccessMiddleware("synchro_credentials", "read"), function(req, res) {
    var id_e_synchro_credentials = req.query.id;
    ...
    entity_helper.optimizedFindOne('E_synchro_credentials', id_e_synchro_credentials, options).then(function(e_synchro_credentials) {
        ...
        entity_helper.getPicturesBuffers(e_synchro_credentials, "e_synchro_credentials").then(function() {
            status_helper.translate(e_synchro_credentials, attributes, req.session.lang_user);
            data.componentAddressConfig = component_helper.getMapsConfigIfComponentAddressExist("e_synchro_credentials");
...
router.get('/create_form', block_access.actionAccessMiddleware("synchro_credentials", "create"), function(req, res) {
    var data = {
        enum_radio: enums_radios.translated("e_synchro_credentials", req.session.lang_user, options)
    };
    ...
    // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
    entity_helper.getLoadOnStartData(data, options).then(function(data) {
        var view = req.query.ajax ? 'e_synchro_credentials/create_fields' : 'e_synchro_credentials/create';
...
router.post('/create', block_access.actionAccessMiddleware("synchro_credentials", "create"), function(req, res) {

    var createObject = model_builder.buildForRoute(attributes, options, req.body);

    models.E_synchro_credentials.create(createObject).then(function(e_synchro_credentials) {
        var redirect = '/synchro_credentials/show?id=' + e_synchro_credentials.id;
        req.session.toastr = [{
        ...
        model_builder.setAssocationManyValues(e_synchro_credentials, req.body, createObject, options).then(function() {
            Promise.all(promises).then(function() {
                component_helper.setAddressIfComponentExist(e_synchro_credentials, options, req.body).then(function() {
...
router.get('/update_form', block_access.actionAccessMiddleware("synchro_credentials", "update"), function(req, res) {
    var id_e_synchro_credentials = req.query.id;
    var data = {
        enum_radio: enums_radios.translated("e_synchro_credentials", req.session.lang_user, options)
    };
    ...
    entity_helper.optimizedFindOne('E_synchro_credentials', id_e_synchro_credentials, options).then(function(e_synchro_credentials) {
        ...
        data.e_synchro_credentials = e_synchro_credentials;
        // Update some data before show, e.g get picture binary
        entity_helper.getPicturesBuffers(e_synchro_credentials, "e_synchro_credentials", true).then(function() {
            // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
            entity_helper.getLoadOnStartData(req.query.ajax ? e_synchro_credentials.dataValues : data, options).then(function(data) {
                if (req.query.ajax) {
...
router.post('/update', block_access.actionAccessMiddleware("synchro_credentials", "update"), function(req, res) {
    var id_e_synchro_credentials = parseInt(req.body.id);
    ...
    models.E_synchro_credentials.findOne({
        ...
        component_helper.updateAddressIfComponentExist(e_synchro_credentials, options, req.body);
        e_synchro_credentials.update(updateObject).then(function() {

            // We have to find value in req.body that are linked to an hasMany or belongsToMany association
            // because those values are not updated for now
            model_builder.setAssocationManyValues(e_synchro_credentials, req.body, updateObject, options).then(function() {

                var redirect = '/synchro_credentials/show?id=' + id_e_synchro_credentials;
                if (typeof req.body.associationFlag !== 'undefined')
                    redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
...
router.get('/loadtab/:id/:alias', block_access.actionAccessMiddleware('synchro_credentials', 'read'), function(req, res) {
    ...
    // Fetch tab data
    models.E_synchro_credentials.findOne(queryOpts).then(function(e_synchro_credentials) {
        if (!e_synchro_credentials)
            return res.status(404).end();

        var dustData = e_synchro_credentials[option.as] || null;
...
router.get('/set_status/:id_synchro_credentials/:status/:id_new_status', block_access.actionAccessMiddleware("synchro_credentials", "read"), block_access.statusGroupAccess, function(req, res) {
    status_helper.setStatus('e_synchro_credentials', req.params.id_synchro_credentials, req.params.status, req.params.id_new_status, req.session.passport.user.id, req.query.comment).then(()=> {
        res.redirect(req.headers.referer);
    }).catch((err)=> {
        entity_helper.error(err, req, res, '/synchro_credentials/show?id=' + req.params.id_synchro_credentials, "e_synchro_credentials");
    });
});

router.post('/search', block_access.actionAccessMiddleware('synchro_credentials', 'read'), function(req, res) {
    ...
    models.E_synchro_credentials.findAndCountAll(where).then(function(results) {
...
router.post('/fieldset/:alias/remove', block_access.actionAccessMiddleware("synchro_credentials", "delete"), function(req, res) {
    var alias = req.params.alias;
    ...
    models.E_synchro_credentials.findOne({
        ...
        e_synchro_credentials['remove' + entity_helper.capitalizeFirstLetter(alias)](idToRemove).then(aliasEntities => {
...
router.post('/fieldset/:alias/add', block_access.actionAccessMiddleware("synchro_credentials", "create"), function(req, res) {
    var alias = req.params.alias;
    var idEntity = req.body.idEntity;
    models.E_synchro_credentials.findOne({
        where: {
        ...
        e_synchro_credentials['add' + entity_helper.capitalizeFirstLetter(alias)](toAdd).then(function() {
...
router.post('/delete', block_access.actionAccessMiddleware("synchro_credentials", "delete"), function(req, res) {
    var id_e_synchro_credentials = parseInt(req.body.id);
    ...
    models.E_synchro_credentials.findOne({
        ...
        deleteObject.destroy().then(function() {
            ...
            var redirect = '/synchro_credentials/list';
            if (typeof req.body.associationFlag !== 'undefined')
                redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
            res.redirect(redirect);
