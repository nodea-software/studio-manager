...
router.post('/datalist', block_access.actionAccessMiddleware("role", "read"), function(req, res) {
    filterDataTable("E_role", req.body).then(function(rawData) {
...
router.get('/set_status/:id_role/:status/:id_new_status', block_access.actionAccessMiddleware("role", "read"), block_access.statusGroupAccess, function(req, res) {
    status_helper.setStatus('e_role', req.params.id_role, req.params.status, req.params.id_new_status, req.session.passport.user.id, req.query.comment).then(()=> {
        res.redirect(req.headers.referer);
    }).catch((err)=> {
        entity_helper.error(err, req, res, '/role/show?id=' + req.params.id_role, "e_role");
    });
});
...
router.post('/fieldset/:alias/remove', block_access.actionAccessMiddleware("role", "delete"), function(req, res) {
    ...
    models.E_role.findOne({
        ...
        e_role['remove' + entity_helper.capitalizeFirstLetter(alias)](idToRemove).then(aliasEntities => {
