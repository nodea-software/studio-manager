...
var attributes = require('../models/attributes/e_robot');
var options = require('../models/options/e_robot');
...
router.get('/list', block_access.actionAccessMiddleware("robot", "read"), function(req, res) {
    res.render('e_robot/list');
});

router.post('/datalist', block_access.actionAccessMiddleware("robot", "read"), function(req, res) {
    filterDataTable("E_robot", req.body).then(function(rawData) {
        entity_helper.prepareDatalistResult('e_robot', rawData, req.session.lang_user).then(function(preparedData) {
...
router.post('/subdatalist', block_access.actionAccessMiddleware("robot", "read"), function(req, res) {
    ...
    models.E_robot.findOne({
        ...
        e_robot['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]().then(function(count) {
            var rawData = {
                recordsTotal: count,
                recordsFiltered: count,
                data: []
            };
            for (var i = 0; i < e_robot[subentityAlias].length; i++)
...
router.get('/show', block_access.actionAccessMiddleware("robot", "read"), function(req, res) {
    var id_e_robot = req.query.id;
    ...
    entity_helper.optimizedFindOne('E_robot', id_e_robot, options).then(function(e_robot) {
        if (!e_robot) {
            data.error = 404;
        ...
        entity_helper.getPicturesBuffers(e_robot, "e_robot").then(function() {
            status_helper.translate(e_robot, attributes, req.session.lang_user);
            data.componentAddressConfig = component_helper.getMapsConfigIfComponentAddressExist("e_robot");
...
router.get('/create_form', block_access.actionAccessMiddleware("robot", "create"), function(req, res) {
    var data = {
        enum_radio: enums_radios.translated("e_robot", req.session.lang_user, options)
    };
    ...
    // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
    entity_helper.getLoadOnStartData(data, options).then(function(data) {
        var view = req.query.ajax ? 'e_robot/create_fields' : 'e_robot/create';
...
router.post('/create', block_access.actionAccessMiddleware("robot", "create"), function(req, res) {

    var createObject = model_builder.buildForRoute(attributes, options, req.body);

    models.E_robot.create(createObject).then(function(e_robot) {
        var redirect = '/robot/show?id=' + e_robot.id;
        req.session.toastr = [{
        ...
        model_builder.setAssocationManyValues(e_robot, req.body, createObject, options).then(function() {
            Promise.all(promises).then(function() {
                component_helper.setAddressIfComponentExist(e_robot, options, req.body).then(function() {
...
router.get('/update_form', block_access.actionAccessMiddleware("robot", "update"), function(req, res) {
    var id_e_robot = req.query.id;
    var data = {
        enum_radio: enums_radios.translated("e_robot", req.session.lang_user, options)
    };
    ...
    entity_helper.optimizedFindOne('E_robot', id_e_robot, options).then(function(e_robot) {
        ...
        data.e_robot = e_robot;
        // Update some data before show, e.g get picture binary
        entity_helper.getPicturesBuffers(e_robot, "e_robot", true).then(function() {
            // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
            entity_helper.getLoadOnStartData(req.query.ajax ? e_robot.dataValues : data, options).then(function(data) {
                if (req.query.ajax) {
...
router.post('/update', block_access.actionAccessMiddleware("robot", "update"), function(req, res) {
    var id_e_robot = parseInt(req.body.id);
    ...
    models.E_robot.findOne({
        ...
        component_helper.updateAddressIfComponentExist(e_robot, options, req.body);
        e_robot.update(updateObject).then(function() {

            // We have to find value in req.body that are linked to an hasMany or belongsToMany association
            // because those values are not updated for now
            model_builder.setAssocationManyValues(e_robot, req.body, updateObject, options).then(function() {

                var redirect = '/robot/show?id=' + id_e_robot;
                if (typeof req.body.associationFlag !== 'undefined')
                    redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
...
router.get('/loadtab/:id/:alias', block_access.actionAccessMiddleware('robot', 'read'), function(req, res) {
    ...
    // Fetch tab data
    models.E_robot.findOne(queryOpts).then(function(e_robot) {
        if (!e_robot)
            return res.status(404).end();

        var dustData = e_robot[option.as] || null;
...
router.get('/set_status/:id_robot/:status/:id_new_status', block_access.actionAccessMiddleware("robot", "read"), block_access.statusGroupAccess, function(req, res) {
    status_helper.setStatus('e_robot', req.params.id_robot, req.params.status, req.params.id_new_status, req.session.passport.user.id, req.query.comment).then(()=> {
        res.redirect(req.headers.referer);
    }).catch((err)=> {
        entity_helper.error(err, req, res, '/robot/show?id=' + req.params.id_robot, "e_robot");
    });
});

router.post('/search', block_access.actionAccessMiddleware('robot', 'read'), function(req, res) {
    ...
    models.E_robot.findAndCountAll(where).then(function(results) {
...
router.post('/fieldset/:alias/remove', block_access.actionAccessMiddleware("robot", "delete"), function(req, res) {
    var alias = req.params.alias;
    ...
    models.E_robot.findOne({
        ...
        e_robot['remove' + entity_helper.capitalizeFirstLetter(alias)](idToRemove).then(aliasEntities => {
...
router.post('/fieldset/:alias/add', block_access.actionAccessMiddleware("robot", "create"), function(req, res) {
    var alias = req.params.alias;
    var idEntity = req.body.idEntity;
    models.E_robot.findOne({
        where: {
        ...
        e_robot['add' + entity_helper.capitalizeFirstLetter(alias)](toAdd).then(function() {
...
router.post('/delete', block_access.actionAccessMiddleware("robot", "delete"), function(req, res) {
    var id_e_robot = parseInt(req.body.id);
