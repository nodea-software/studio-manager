...
router.post('/datalist', block_access.actionAccessMiddleware("group", "read"), function(req, res) {
    filterDataTable("E_group", req.body).then(function(rawData) {
...
router.get('/set_status/:id_group/:status/:id_new_status', block_access.actionAccessMiddleware("group", "read"), block_access.statusGroupAccess, function(req, res) {
    status_helper.setStatus('e_group', req.params.id_group, req.params.status, req.params.id_new_status, req.session.passport.user.id, req.query.comment).then(()=> {
        res.redirect(req.headers.referer);
    }).catch((err)=> {
        entity_helper.error(err, req, res, '/group/show?id=' + req.params.id_group, "e_group");
    });
});
...
router.post('/fieldset/:alias/remove', block_access.actionAccessMiddleware("group", "delete"), function(req, res) {
    ...
    models.E_group.findOne({
        ...
        e_group['remove' + entity_helper.capitalizeFirstLetter(alias)](idToRemove).then(aliasEntities => {
